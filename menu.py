...
def menu_loop():
    ...
    font = pygame.font.SysFont('Calibri', TILE_SIZE)
    ...
    characters = [("Player", Algorithm.PLAYER), ("DFS", Algorithm.DFS),
                  ("A*", Algorithm.A_STAR), ("None", Algorithm.NONE)]
    play_options.add.selector(
        "Nhân vật 1", characters, onchange=change_player)
    play_options.add.selector(
        "Nhân vật 2", characters, onchange=change_enemy1, default=2)
    play_options.add.selector(
        "Nhân vật 3", characters, onchange=change_enemy2, default=2)
    play_options.add.selector(
        "Nhân vật 4", characters, onchange=change_enemy3, default=2)
    play_options.add.selector(
        "Xem đường đi", [("Có", True), ("Không", False)], onchange=change_path)

    play_options.add.button('Trở lại', pygame_menu.events.BACK)
    play_menu.add.button('Bắt đầu', run_game)

    play_menu.add.button('Tùy chọn', play_options)
    play_menu.add.button('Trở lại', pygame_menu.events.BACK)
    ...
    tutorial_menu.add.label("Điều khiển người chơi: ")
    tutorial_menu.add.label("Di chuyển: WSAD")
    tutorial_menu.add.label("Đặt bom: Space")
    tutorial_menu.add.vertical_margin(25)
    tutorial_menu.add.button('Trở lại', pygame_menu.events.BACK)
    ...
    main_menu.add.button('Chơi', play_menu)
    main_menu.add.button('Hướng dẫn', tutorial_menu)
    main_menu.add.button('Thoát', pygame_menu.events.EXIT)
